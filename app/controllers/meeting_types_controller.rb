class MeetingTypesController < AuthenticationController
  ...
  def create
    ...
    respond_to do |format|
      if @meeting_type.save
        ...
        format.json { redirect_to action: "index", status: :created, location: @meeting_type }
      else
  ...
  def update
    respond_to do |format|
      if @meeting_type.update(meeting_type_params)
        ...
        format.json { redirect_to action: "index", status: :ok, location: @meeting_type }
      else
  ...
  private
    ...
    def meeting_type_params
      params.require(:meeting_type).permit(:name, :icon, :default_color)
    end
end
